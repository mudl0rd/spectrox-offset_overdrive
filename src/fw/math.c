...
float fw_math_randf(float min, float max) {
    ...
    float rnd = ((float) musys_rand()) / (float) RAND_MAX;
...
int fw_math_rand(int min, int max) {
    ...
    return min + musys_rand() / (RAND_MAX / (max - min + 1) + 1);
}

void fw_math_normalize(fw_vec3f *v) {
    float len = musys_sqrtf(v->x * v->x + v->y * v->y + v->z * v->z);
    v->x /= len;
    v->y /= len;
    v->z /= len;
}
...
float fw_math_len(fw_vec3f *v) {
    return musys_sqrtf(fw_math_lenSquared(v));
}
...
float fw_math_damp(float src, float dest, float lambda, float dt) {
    ...
    return fw_math_lerp(src, dest, 1.f-musys_exp(-lambda*dt));
}
